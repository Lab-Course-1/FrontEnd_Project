...
const CreateOrder = () => {
  ...
  const handleSubmit = async (event) => {
    if (
      ...
      showWarningNotification('Please fill in all fields!', 'Enter valid data.', 2000);
      return;
    }
    ...
    try {
      const response = await axios.post(
        Variables.API_URL +
          `Order/CreateOrderFromShoppingCard${
            promotion && promotion !== '' ? `?promoCode=${promotion}` : ''
          }`,
        {
          phoneNumber,
          name,
      ...
      if (response.status === 200) {
        ...
        setTimeout(() => {
          navigateToCompletedPage(data.products, data.numberOfProducts);
        }, 2000);
      }
    } catch (error) {
  ...
  const navigateToCompletedPage = (products, numberOfProducts) => {
    navigate('/completed', {
      state: {
        products,
        numberOfProducts,
